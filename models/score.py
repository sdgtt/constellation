...
from models.boot_tests import BoardBootTests
    ...
    def get_artifacts(self):
        ...
        for build_no in self.builds:
            artifacts[build_no] = Artifacts(
                job=self.jenkins_project,
                job_no=build_no,
                target_board=self.board
            ).artifacts
            ...
            for bt in bts:
                for test in tests:
                    ...
                    if test == "boards":
                        ...
                        if not bt.boot_folder_name in report[bn][test]['data']:
                        ...
                        if not bt.linux_prompt_reached:
                            report[bn][test]['count'] += 1
                            if not bt.last_failing_stage_failure\
                                in report[bn][test]['data']:
                                report[bn][test]['data']\
                                [bt.last_failing_stage_failure] = [bt.boot_folder_name]
                            else:
                                report[bn][test]['data']\
                                [bt.last_failing_stage_failure].append(bt.boot_folder_name)
                                ...
                                if ar.artifact_info_type == supported_artifacts[test]:
                                    if ar.target_board == bt.boot_folder_name:
                                        if ar.payload in report[bn][test]['data']:
                                            report[bn][test]['data'][ar.payload].append(ar.target_board)
                                        else:
                                            report[bn][test]['data'].update({ar.payload: [ar.target_board]})
            ...
            for f in regression_fields:
                if f == "regress_missing_logs":
                    regressions[str(build)][f] = \
                        self.__get_removed(
                            self.boards,
                            self.test_results[str(build)]["boards"]["data"]
                        )
                    ...
                    if index == 0:
                        ...
                        r, a, u = self.__get_diff(
                                    self.test_results[sorted_builds[index-1][1]]["boards"]["data"],
                ...
                elif f in ["regress_not_booted_added",
                            "regress_not_booted_removed",
                            "regress_not_booted_unresolved",
                            "regress_drivers_missing_added",
                            "regress_drivers_missing_removed",
                    ...
                    if index == 0:
                        ...
                        r_type = f.split("_")[3]
                        if len(f.split("_")) > 4:
                            info = f.split("_")[1] + '_' + f.split("_")[2] + '_' + f.split("_")[3]
                            r_type = f.split("_")[4]
                            
                        r, a, u = self.__get_diff(
                            self.test_results[sorted_builds[index-1][1]][info]["data"].keys(),
                            self.test_results[build][info]["data"].keys()
                        )
                        regressions[str(build)][f] = {}
                        if r_type == "removed":
                            for removed in r:
                                failing_boards = self.test_results[sorted_builds[index-1][1]][info]["data"][removed]
                                regressions[str(build)][f].update({removed:failing_boards})
                        elif r_type == "added":
                            for added in a:
                                failing_boards = self.test_results[build][info]["data"][added]
                                regressions[str(build)][f].update({added:failing_boards})
                        elif r_type == "unresolved":
                            for unresolved in u:
                                failing_boards = self.test_results[build][info]["data"][unresolved]
                                regressions[str(build)][f].update({unresolved:failing_boards})

            # print(build)
            # for _f in regression_fields:
            #     print("{}: {}".format(_f, regressions[build][_f]))
        return regressions
